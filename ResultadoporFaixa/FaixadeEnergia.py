...
for occ, df_occ in datasets.items():
    ...
    df_analysis = pd.DataFrame({
        ...
        'Amplitude_Predita': y_test_pred
    })

    df_analysis['Erro'] = (
        df_analysis['Amplitude_Real']
        - df_analysis['Amplitude_Predita']
    )

    df_analysis['Erro_Abs'] = np.abs(
        df_analysis['Erro']
    )
    ...
    plt.savefig(
        f"resultados/graficos/residuo_vs_energia_OCC_{occ}.png",
        dpi=300,
        bbox_inches='tight'
    )
    plt.show()
    ...
    plt.title(
        f'Resíduos por Faixa de Energia - OCC {occ}'
    )
    ...
    plt.savefig(
        f"resultados/graficos/boxplot_residuos_OCC_{occ}.png",
        dpi=300,
        bbox_inches='tight'
    )
    plt.show()
...
plt.savefig(
    "resultados/graficos/heatmap_mae_energia.png",
    dpi=300,
    bbox_inches='tight'
)
plt.show()
...
rn_residuals.to_csv(
    "resultados/residuos/rn_residuals.csv",
    index=False
)

heatmap_df.to_csv(
    "resultados/stats/mae_por_faixa_energia.csv"
)
